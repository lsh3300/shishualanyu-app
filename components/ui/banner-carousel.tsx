    ...
    <div className="relative w-full aspect-[21/9] sm:aspect-[18/7] rounded-xl overflow-hidden shadow-lg group my-2">
      ...
      >
        {items.map((item, index) => (
          <div 
            key={item.id} 
            onClick={() => handleSlideClick(item.href)}
            className="w-full h-full flex-shrink-0 relative group-hover:scale-[1.02] transition-transform duration-700 hover:shadow-xl cursor-pointer"
          >
            <Image 
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 transition-all duration-500 transform translate-y-0">
              ...
              {/* 查看详情按钮 - 使用 span 代替 Button 避免嵌套问题 */}
              <div className="animate-fadeIn animation-delay-400">
                <span className="inline-flex items-center bg-background text-primary hover:bg-background/90 transition-all rounded-full px-6 py-2 shadow-md hover:shadow-lg text-sm font-medium">
                  查看详情
