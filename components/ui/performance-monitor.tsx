...
class PerformanceMonitor {
  ...
  getWebVitals(): {
    ...
    if (clsEntries.length > 0) {
      vitals.CLS = clsEntries.reduce((sum, entry) => sum + (entry.value ?? 0), 0)
    }

    // First Input Delay (FID)
    const fidEntries = performance.getEntriesByType("first-input") as Array<PerformanceEntry & { processingStart?: number; startTime: number }>
    if (fidEntries.length > 0) {
      const first = fidEntries[0]
      if (typeof first.processingStart === "number") {
    ...
    <>
      ...
      {isVisible && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-lg max-w-[calc(100%-2rem)] sm:max-w-2xl w-full max-h-[80vh] overflow-auto p-4 sm:p-6 my-4">
