  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      if (normalizedMessage.includes("database error saving new user")) {
        setErrors("注册失败：数据库触发器/用户资料表配置异常（profiles）。请先在 Supabase SQL Editor 执行项目里的 signup 修复脚本后再重试。")
      } else {
        setErrors(rawMessage || "注册失败，请稍后再试")
      }
      setIsLoading(false)
    } else {
  ...
  return (
    <div className="p-8">
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-2">加入我们</h2>
        <p className="text-sm text-gray-600">创建账户，开启蓝染文化之旅</p>
      </div>
