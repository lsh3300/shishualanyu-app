...
export async function GET(request: NextRequest) {
  try {
    ...
    // 格式化用户上架的作品
    const formattedUserListings = (userListings || []).map((listing: any) => {
      const userShops = listing?.user_shops as any
      const shop = Array.isArray(userShops) ? userShops[0] : userShops

      return {
        id: listing.id,
        shop_id: shop?.id,
        seller_id: shop?.user_id,
        seller_name: shop?.shop_name || '未知商店',
