...
export async function GET(request: NextRequest) {
  try {
    ...
    // 方法1: 使用数据库函数（如果迁移已执行）
    try {
      const { data: achievements, error } = await serviceSupabase
    ...
    // 2. 进行中课程数
    const { count: inProgressCount } = await serviceSupabase
      .from('enrollments')
      .select('*', { count: 'exact', head: true })
      .eq('user_id', user.id)
    ...
    // 4. 点赞数
    const { count: likesCount } = await serviceSupabase
      .from('course_likes')
      .select('*', { count: 'exact', head: true })
      .eq('user_id', user.id)
    
    // 5. 评论数
    const { count: commentsCount } = await serviceSupabase
