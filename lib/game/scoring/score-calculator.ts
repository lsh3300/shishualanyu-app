...
function calculateLayoutCreativity(layers: ClothLayer[]): number {
  ...
  // 有分散更好（不是全部堆在一起）
  const spreadScore = Math.min((xSpread + ySpread) / 200, 1)
...
export function calculateTechniqueScore(layers: ClothLayer[]): number {
  ...
  if (layers.length > 1) {
    ...
    const isProgressive = dyeDepths.every((depth, i) => {
      if (i === 0) return true
      return depth >= dyeDepths[i - 1] - 0.1 // 允许轻微回退
    })
  ...
  // 3. 纹样搭配（30分）
  // 检查纹样组合是否和谐
  const hasVariety = new Set(layers.map(l => l.textureId)).size > 1
  const notTooMany = new Set(layers.map(l => l.textureId)).size <= layers.length * 0.8
...
export function calculateTotalScore(
  ...
  if (total >= 95) grade = 'SSS'
  else if (total >= 90) grade = 'SS'
  else if (total >= 80) grade = 'S'
...
export function scoreCloth(layers: ClothLayer[]): {
  ...
  // 计算各维度分数
  const color_score = 85 // 暂时使用固定值，实际需要对比目标颜色
