...
import { ShopConfig } from '@/lib/game/config'
import type {
  UserShop,
  ShopListing,
  Transaction,
...
export async function getShopWithOwner(shopId: string): Promise<ShopWithOwner | null> {
  ...
  try {
    ...
    return {
      ...shop,
      owner_name: `用户${(profile?.user_id || shop.user_id).substring(0, 8)}`
    }
  } catch (error) {
...
export async function createListing(
  ...
  try {
    ...
    const basePrice =
      scoreResult.data &&
      typeof (scoreResult.data as { total_score?: unknown }).total_score === 'number' &&
      typeof (scoreResult.data as { grade?: unknown }).grade === 'string'
        ? calculatePriceFromScore(
            (scoreResult.data as { total_score: number }).total_score,
            (scoreResult.data as { grade: string }).grade as ScoreGrade
          )
        : price
