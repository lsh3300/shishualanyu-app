...
export function usePlayerProfile(): UsePlayerProfileReturn {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      if (authError) {
        console.error('认证错误:', authError.message, authError)
      ...
      if (dbError) {
        ...
        if (dbError.code === 'PGRST116') {
          ...
          if (createError) {
            console.error('创建档案失败:', createError.message, createError)
            setError(`创建玩家档案失败: ${createError.message}`)
            setLoading(false)
            return
          }
          ...
          console.error('查询档案失败:', dbError.message, dbError)
          setError(`加载玩家档案失败: ${dbError.message}`)
          setLoading(false)
          return
        }
      } else {
